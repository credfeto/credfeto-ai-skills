...
6. **On failure**, check the round cap first, then judge the round-over-round trend:
   ...
   - **Flat or worsening, but more rounds are still judged worth trying**: post the status comment as in the gap-closing case above, update the workflow board to a "development" status if one is configured, and stop. Do not write `COVERAGE.md` in this case.
