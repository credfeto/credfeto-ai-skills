...
5. **On success**: write/overwrite `COVERAGE.md` with the numbers just measured, commit and push it, update the workflow board to a "human review" status if one is configured, post a one-line status comment (`Coverage ratchet passed - advancing to Human Review`), and stop.
