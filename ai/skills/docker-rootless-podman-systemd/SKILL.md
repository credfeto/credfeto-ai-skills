---
name: credfeto-docker-rootless-podman-systemd
description: Configure or debug rootless Podman that is run by a system systemd unit (a unit file under /etc/systemd/system, activated with WantedBy=multi-user.target) under an unprivileged User= account that has no real login session, as opposed to a systemctl --user unit or an interactively logged-in account. Use whenever writing or troubleshooting such a unit, its install/upgrade scripts, or its compose-based deployment, including D-Bus/networking failures, cgroup permission errors, container kills on restart, home-directory sandboxing pitfalls, timer migrations, bind-mounted secret permissions, firewalld interactions, image pruning, or podman-compose subcommand differences.
---

# Rootless Podman Under an Unprivileged System Systemd Service

This applies specifically to a **system** systemd unit (`/etc/systemd/system/*.service`, `WantedBy=multi-user.target`) running rootless Podman under an unprivileged `User=` account with **no real login session**: not a `systemctl --user` unit, and not an interactively logged-in account. This is the shape produced when "run this as an unprivileged systemd service, not a user service" is the explicit requirement.
...
- Fix: use `ProtectSystem=strict` instead; drop `ProtectHome`. Do **not** add `NoNewPrivileges=yes` alongside this; see the next section for why. Do **not** add `PrivateTmp=yes` either; see the `PrivateTmp=yes` section below for why.
...
- Rootless Podman keeps a long-lived "pause" process per UID (`/run/user/<uid>/libpod/tmp/pause.pid`, cgroup `podman-pause-*.scope`, reparented to PID 1) so it doesn't have to rebuild the user namespace on every invocation: the same mechanism the `NoNewPrivileges=yes` section above depends on surviving across restarts within a boot. `PrivateTmp=yes` gives every *start* of the container-runner unit its own private `/tmp` and `/var/tmp` bind mounts, torn down when that particular service instance ends.
...
- A sibling file bind-mounted with the same `640` permissions into a different container working fine is not evidence the general problem does not apply; that is almost certainly a coincidence of whatever UID that particular container's process happens to map to. Do not treat one working case as proof `640` is safe for bind-mounted secrets under rootless Podman in general.
...
- Check `firewall-cmd --get-active-zones` (not just `--list-all` on the zone you assume matters) before concluding a specific port-open rule is or is not the cause of connectivity behaviour on a host that has ever run docker.
...
- Make it non-fatal (`|| true`): by the time pruning runs, the containers are already up, so a transient prune failure (empty store, a lock, etc.) must never fail the whole deploy/update run over what is purely best-effort disk cleanup.
