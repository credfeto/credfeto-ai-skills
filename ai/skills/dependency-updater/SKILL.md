...
- If you take over or push any commit to a Dependabot (or other bot) PR and its changelog-check CI job then fails, do not assume the bot's `Changelog Not Required` label still applies once you have pushed to the PR: add the missing changelog entry yourself, describing the dependency bump, exactly as you would for a change you authored yourself, and verify the CI check actually passes afterwards.
