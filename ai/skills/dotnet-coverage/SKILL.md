...
- For methods whose success path requires live infrastructure (database connections, network sockets, file handles), do not suppress PH2140 or add `[ExcludeFromCodeCoverage]` — test the failure paths and leave the infrastructure-dependent success path to integration tests.
